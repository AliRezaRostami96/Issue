...
<div class="main-menu position-fixed top-0 bottom-0 start-0 shadow text-white">
    <div class="position-relative h-100 w-100 p-2">
        <button
            class="btn-sm shadow-none position-absolute start-100 translate-middle-x text-dark bg-white border-0"
            onclick="menuToggle()"
        >
            <i class="fas fa-bars h5 m-0 py-1"></i>
        </button>
        <div class="text-center invisible mb-3 logo">
            <img class="img-fluid px-5" src="https://issue.fajriff.com/assets/img/brand/blue.png" alt="logo">
        </div>
        <ul class="list-unstyled">
            @foreach($lists as $item)
                <li class="mt-3 pb-2 menu-item">
                    <a href="/{{$item['PageLink']}}"
                       class="text-decoration-none d-inline-block w-100 h-100 text-nowrap overflow-hidden {{(request()->is($item['PageLink'])) ? 'text-dark' : 'text-black-50' }}">
